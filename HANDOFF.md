...
**작업 방식 안내**: 사용자가 "배포"라고 말하면, Claude는 매번
① 최근 변경사항 코드 점검(에러/버그/보안) → ② 이 문서(HANDOFF.md)와 README.md를 최신
상태로 갱신 → ③ 커밋·푸시(Vercel/Render 자동 배포)까지 알아서 진행하기로 되어있음
(2026-08-20 사용자 지시). 점검 중 발견한 문제는 고치고 나서 진행함.
