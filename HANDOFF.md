...
- 사용자가 **"배포"라고 명시적으로 말할 때만** ① 그때까지 쌓인 변경사항(커밋된 것들) 코드
  점검(에러/버그/보안) → ② 이 문서(HANDOFF.md)와 README.md를 최신 상태로 갱신 → ③ 푸시
  (Vercel/Render 자동 배포)까지 한 번에 진행함. 점검 중 발견한 문제는 고치고(커밋 추가)
  나서 진행.
