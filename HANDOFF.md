...
- **인기 검색어 기능** — 검색 결과를 클릭해서 카드로 추가할 때(타이핑 중간값은 제외) Upstash
  ...
  - **UI는 검색창 아래 칩이 아니라 타이틀바 세로 슬라이딩 티커**로 최종 확정됨
    ...
    2026-08-20에 제거됨.
  - **기본 화면뿐 아니라 엑셀/워드/PPT 위장 모드 제목표시줄에도 같은 티커가 뜸** —
    위장 디자인을 해치지 않으려고 알약(pill) 배경 없이 각 제목표시줄 색 위에 텍스트만
    얹는 식으로 스타일링. `app.js`의 `TICKER_INSTANCES` 배열이 4곳(기본/엑셀/워드/PPT)의
    엘리먼트를 한 번에 들고 있고, 페인트/슬라이드 전환/호버 패널 로직이 전부 이 배열을
    순회하며 동시에 적용됨 — 새 위장 테마를 추가하게 되면 이 배열에 항목만 추가하면 됨.
...
- **테마 전환 메뉴 UX 개선** — 원래 아이콘 하나뿐이라 눌러야 메뉴가 나오는지 알기 어려웠음.
  기본 화면은 아이콘을 "테마 ⌄" 텍스트+화살표 버튼(`.skin-menu-btn`)으로 바꿔서 메뉴라는 걸
  바로 알 수 있게 함. 위장 모드(엑셀/워드/PPT) 안의 작은 로고 버튼(X/W/P)은 진짜 앱처럼
  보여야 해서 대놓고 메뉴 버튼처럼 만들 순 없으니, 로고 코너에 아주 작은 점 배지만 둬서
  위장은 유지하면서도 눌러볼 수 있다는 걸 알아챌 수 있게 함.
